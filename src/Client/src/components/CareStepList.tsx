...
import type { CareStep, CareStepAction, UserRole } from '../types';
import { STEP_STATUS_COLOR, stepPrefix } from '../constants/careSteps';
import { canEnterStep } from '../constants/roles';
...
interface Props {
  ...
  /** The current user's roles — gates the "enter" (admit) action to the wait that targets them. */
  userRoles?: UserRole[];
 ...
 * The "enter" (admit) action is shown only for the wait that targets the current user's track
 * (a doctor can't admit to a nurse's wait, and vice versa). The doctor "claim" (responsible party)
 * lives in its own column on the queue page, not here.
 */
  ...
  return (
    <Stack gap={4}>
      {ordered.map((s) => {
        ...
        // The "enter" button is shown only for the wait that targets the current user's track.
