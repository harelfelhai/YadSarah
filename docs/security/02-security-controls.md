...
## 25. טופס-טיפול: הצגת שדות התפקיד-השני לקריאה (א-סימטרי) (2026-06-28..29)

שינוי-**תצוגה/פריסה בלקוח בלבד** (`features/treatment/TreatmentFormPage.tsx`) לסקשנים שמולאו ע"י התפקיד
השני, **לקריאה-בלבד**, בפריסה א-סימטרית לפי התפקיד:
- **מבט-רופא** (מי שרשאי לערוך את "סיבת הפנייה — רופא"): "סיבת הפנייה — אחות" של האחות מוצגת read-only
  **בראש העמוד כברירת-מחדל** (היא הסקשן הראשון).
- **מבט-אחות**: סקשני-הרופא המלאים **מוסתרים כברירת-מחדל** ונחשפים read-only רק בלחיצה על כפתור "הצג שדות
  רופא" **בתחתית** הטופס שלה.
...
- **אין PHI/גישה חדשים:** השרת כבר החזיר את **כל** הסקשנים (`FormsController.MapForm` — ללא רידקציה-
  לפי-תפקיד) לכל צוות-קליני המורשה לצפות בביקור. השינוי הוא **סדר-הצגה וברירת-מחדל של גילוי בלבד** —
  מידע קליני חיוני לאותו צוות-טיפול (הרופא חייב לראות את סיבת-ההפניה של האחות).
...
- **חלון-התור מציג מטופלים לא-משוחררים מעבר לגבול-האיפוס, חסום לחלון יום-התור הנוכחי+הקודם
  (`VisitService.GetQueueAsync`):** קודם הסינון היה `AdmissionDate == queueDate` בלבד, כך שביקורים פתוחים
  מ"יום-התור" הקודם נעלמו מהלוח בשעת-האיפוס. כעת הלוח מציג ביקורים לא-משוחררים מ-`AdmissionDate >=
  queueDate-1` (ו-`includeDischarged` מוסיף את משוחררי-היום) — מטופל-ערב שורד את גלגול-06:00 בלי שהלוח
  יצבור את כל ההיסטוריה של הלא-משוחררים (ראה ui-probe 2026-06-29: ללא החסם הוצגו ~590 "פעילים" מצטברים).
  **אין מחלקת-PHI חדשה ואין נתיב חדש** — אותו `GET /api/visits/queue` המוגן-RBAC, רק קריטריון-סינון על
  אותה רשומה. רק המספור היומי מתאפס (`QueueCounter`), לא רשימת-המטופלים. ברירת-המחדל של
...
## 29. אתחול admin ראשון (bootstrap) בסביבה ריקה (2026-07-01)

בסביבת-פרודקשן חדשה טבלת ה-Users **ריקה** (זריעת-המשתמשים היחידה היא ב-Demo, שחסום כפליים
Demo+Admin — chicken-and-egg), ולכן לא הייתה דרך ליצור את המנהל הראשון. נוסף אתחול חד-פעמי ב-startup
(`Program.cs`, אחרי המיגרציות) היוצר משתמש **Admin** יחיד **אך ורק כאשר טבלת ה-Users ריקה**.

**בקרות-אבטחה על הנתיב הזה:**
- **הסיסמה מגיעה אך-ורק מ-config** (`Bootstrap:AdminPassword`, מוזרק כ-secret מ-Secrets Manager) —
  **אין סיסמת-ברירת-מחדל/קשיחה בקוד.** אם ההגדרה חסרה, **לא נוצר כלום** ונרשמת אזהרה. כך אין סיכון
  של admin עם סיסמה ידועה שנוצר בטעות.
- **אכיפת-מורכבות זהה לכל הנתיבים:** הסיסמה עוברת `PasswordPolicy.Validate` (≥12 תווים + אות-גדולה/
  קטנה/ספרה/תו-מיוחד, תקן ישראלי) בדיוק כמו `UserService` — **סיסמה חלשה נדחית ולא נוצר admin** (נרשמת
  שגיאה בלי לחשוף את הערך). אין נתיב-עוקף למדיניות.
- **אידמפוטנטי:** מדלג ברגע שקיים ולו משתמש אחד — לא ניתן להשתמש בו לדריסה/הסלמה על DB מאוכלס.
- **הסיסמה לעולם אינה נרשמת ליומן** (רק שם-המשתמש, לצורך תפעול).
- **גיבוב:** אותה מדיניות כשאר המשתמשים — `AuthService.HashPassword` (BCrypt, workFactor 12).
- **תיעוד ב-audit:** יצירת ה-admin נרשמת ל**יומן-הביקורת** (`AuditService.LogAsync`, פעולה `Created`,
  ישות `User`, מיוחסת ל-`system`) — כמו נתיב יצירת-המשתמש הרגיל, לרשומת-בראשית של החשבון בעל-ההרשאות.
- **הרשאות:** התוצאה היא משתמש Admin רגיל הכפוף לכל בקרות ה-RBAC/audit/lockout הקיימות; אין נתיב-על.
  *(שני הפריטים לעיל — אכיפת-מורכבות ותיעוד-audit — נוספו בעקבות סקירת-אבטחה אדוורסרית על הרכיב.)*
- **נוהל-הפעלה:** המנהל **מחליף את הסיסמה בכניסה הראשונה ומסיר את `Bootstrap__AdminPassword`** לאחר
  שנוצר המשתמש (הערך הופך רדום ממילא — האתחול מדלג כשהטבלה אינה ריקה).

קבצים: `Api/Program.cs`.
